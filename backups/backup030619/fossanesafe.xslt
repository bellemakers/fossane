...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
			...
			<xsl:for-each select="fossane/page">
				...
				<xsl:document method="html" href="{$documenthref}">
					<html lang="{/fossane/languages/language[@lang=$currentlang]/@shortname}">
						...
						<body marginwidth="0" marginheight="0" bgcolor="#a5ad94">
							<!-- BEGIN header -->
										...
										<table cellpadding="4" cellspacing="0" border="0" width="100%">
											<tbody>
												<tr valign="top">
													...
													<td colspan="2" bgcolor="#7d8868" align="left">
														<div class="heading">
												...
												<tr valign="top">
													...
													<td colspan="2" align="justify" bgcolor="#7d8868">
														<!-- START main column -->
														<div class="colM">
	...
	<xsl:template name="logo">
		...
		<xsl:choose>
			<xsl:when test="$currentlang=1">
				<a href="{$backtoroot}{//filestructure/file/filename[@lang=$currentlang]}.html">
					<img src="{$backtoroot}{$graphicsfolder}logo.gif" alt="Fossane" border="0"/>
				</a>
			</xsl:when>
			<xsl:otherwise>
				<a href="{$backtoroot}{$languagefolder}{//filestructure/file/filename[@lang=$currentlang]}.html">
						...
						<xsl:for-each select="//filestructure/file/file[descendant-or-self::node()/@id=$currentid]/file">
							...
							<xsl:if test="//file[@id=$currentid]=.//file or //file[@id=$currentid]=.">
								<xsl:if test="$currentid=//filestructure/file/file/file/descendant-or-self::node()/@id">
									<tr>
										<td>
											<table bgcolor="#F2F3ED" cellpadding="5" cellspacing="0" width="100%">
												<tbody>
		...
		<xsl:if test="normalize-space(//page[@id=$currentid]/pics/introduction/UL) or normalize-space(//page[@id=$currentid]/pics/introduction/UR)">
			<div class="pics">
				<xsl:if test="normalize-space(//page[@id=$currentid]/pics/introduction/UL)">
					<div class="UL">
						<img src="{$backtoroot}{$graphicsfolder}{//page[@id=$currentid]/pics/introduction/UL}.{//page[@id=$currentid]/pics/introduction/UL/@type}" alt="" border="1" align="left" hspace="5" vspace="5"/>
					</div>
				</xsl:if>
				<xsl:if test="normalize-space(//page[@id=$currentid]/pics/introduction/UR)">
					<div class="UR">
						<img src="{$backtoroot}{$graphicsfolder}{//page[@id=$currentid]/pics/introduction/UR}.{//page[@id=$currentid]/pics/introduction/UR/@type}" alt="" border="1" align="right" hspace="5" vspace="5"/>
					</div>
				</xsl:if>
			</div>
		</xsl:if>
				...
				<xsl:if test="$paranumber=//page[@id=$currentid]/pics/para[UL]/@number or $paranumber=//page[@id=$currentid]/pics/para[UR]/@number">
					<div class="pics">
						<xsl:if test="$paranumber=//page[@id=$currentid]/pics/para[UL]/@number">
							<div class="UL">
								<img src="{$backtoroot}{$graphicsfolder}{//page[@id=$currentid]/pics/para[@number=$paranumber]/UL}.{//page[@id=$currentid]/pics/para[@number=$paranumber]/UL/@type}" alt="" border="1" align="left" hspace="5" vspace="5"/>
							</div>
						</xsl:if>
						<xsl:if test="$paranumber=//page[@id=$currentid]/pics/para[UR]/@number">
							<div class="UR">
								<img src="{$backtoroot}{$graphicsfolder}{//page[@id=$currentid]/pics/para[@number=$paranumber]/UR}.{//page[@id=$currentid]/pics/para[@number=$paranumber]/UR/@type}" alt="" border="1" align="right" hspace="5" vspace="5"/>
							</div>
						</xsl:if>
					</div>
				</xsl:if>
	...
	<xsl:template name="maparea">
		...
		<xsl:for-each select="text/map">
			...
			<p/>
			<img src="{$backtoroot}{$graphicsfolder}{gifname[@lang=$currentlang]}.gif" usemap="#{name}" border="0"/>
			<map name="{name}">
